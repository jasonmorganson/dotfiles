...
let-env PATH = ($env.PATH | split row (char esep) | prepend '/usr/local/bin' | prepend '/opt/homebrew/bin' | prepend '/home/linuxbrew/.linuxbrew/bin' | prepend $"($env.CARGO_HOME)/bin" | prepend $"($env.HOME)/.tea/tea.xyz/v*/bin" | prepend $"($env.HOME)/.local/share/rtx/bin" | prepend $"($env.HOME)/.local/share/rtx/installs/rust/latest/bin")
