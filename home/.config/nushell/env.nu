...
export def --env --wrapped main [command?: string, --help, ...rest: string] {
  ...
  if ($command == null) {
    ^"/Volumes/Home/jasonmorganson/.local/bin/mise"
  } else if ($command == "activate") {
    $env.MISE_SHELL = "nu"
  } else if ($command in $commands) {
    ^"/Volumes/Home/jasonmorganson/.local/bin/mise" $command ...$rest
    | parse vars
    | update-env
  } else {
    ^"/Volumes/Home/jasonmorganson/.local/bin/mise" $command ...$rest
...
def --env mise_hook [] {
  ^"/Volumes/Home/jasonmorganson/.local/bin/mise" hook-env -s nu
    | parse vars
    | update-env
}
